...
export async function renderOfferText(
  text: string, lang: Language, maxW: number, maxH: number
): Promise<Uint8Array> {
  await ensureFonts()
  const isAR = lang === 'AR'
  ...
  const lines = wrapText(ctx, text, maxW - 64)
  const grad = ctx.createLinearGradient(0, 0, 0, lines.length * lineH)
  for (const s of OFFER_TEXT_GRADIENT.stops) grad.addColorStop(s.offset, s.color)
  ctx.fillStyle = grad

  const x = isAR ? maxW - 32 : 32
  lines.forEach((line, i) => ctx.fillText(line, x, (i + 1) * lineH))
